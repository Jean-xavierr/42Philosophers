...
/*   Updated: 2020/09/17 13:32:29 by jereligi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		parsing_nb_philo(int nb)
{
	...
	if (nb > 200)
	{
		...
		printf("very large number of philosophers, ");
		printf("risk of slowing down or crashing.\n");
	}
	return (0);
}
...
int		parsing_time_to(int time_to, char *s)
{
	...
	{
		printf(COLOR_RED "Error : " COLOR_RESET);
		printf("%s can't be less than 1.\n", s);
		return (1);
	}
	else if (time_to > 0 && time_to < 60)
	{
		printf(COLOR_YELLOW "Warning : " COLOR_RESET);
		printf("%s less 60ms delay could be not enough for good result.\n", s);
	}
	return (0);
}
